...
const Product = () => {
  ...
  const getWhatsAppLink = () => {
    if (!selectedSizeData) return `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Ciao! Sono interessato a "${product.name}"`)}`;
...
Grazie!`;
  ...
  const getEmailLink = () => {
    ...
    if (!selectedSizeData) return `mailto:${contactEmail}?subject=${subject}`;
          ...
          <div className="space-y-6">
            ...
            {!isComingSoon && (
              <div className="flex flex-col sm:flex-row gap-3 pt-3 border-t border-border">
                <span className="text-sm text-muted-foreground w-full">Contattami / Supporto</span>
                <a
